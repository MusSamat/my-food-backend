...
router.get('/', asyncHandler(async (req, res) => {
    const { status, type, date_from, date_to, search, page = 1, limit = 20 } = req.query;
    ...
    if (search && search.trim().length >= 1) {
        params.push(`%${search}%`);
        conditions.push(`(
            o.id::text ILIKE $${params.length} OR
            o.name ILIKE $${params.length} OR
            o.phone ILIKE $${params.length}
        )`);
    }
