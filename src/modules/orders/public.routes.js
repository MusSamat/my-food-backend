...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const result = await pool.transaction(async (client) => {
        ...
        const { rows: [order] } = await client.query(
            `INSERT INTO orders (telegram_user_id, telegram_username, type, name, phone,
             ...
             RETURNING *`, // СТАТУСЫ: 'paid' и 'completed' вместо 'pending'
        ...
        await client.query(
            `INSERT INTO order_status_history (order_id, status, changed_by) VALUES ($1, 'paid', 'system')`,
            [order.id]
        );
    ...
    // ─── БЛОК ПЛАТЕЖА ЗАКОММЕНТИРОВАН ДЛЯ ТЕСТОВ ───
    /*
    try {
